...
RUN /usr/lib/ckan/default/bin/pip install -e 'git+https://github.com/ckan/ckan.git@ckan-2.9.9#egg=ckan'
...
RUN ln -s /etc/ckan/default/ckan.ini /usr/lib/ckan/default/src/ckan/ckan.ini 
EXPOSE 5000
COPY ckan-entrypoint /ckan-entrypoint
